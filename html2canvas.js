...
var parseBackground = exports.parseBackground = function parseBackground(style, imageLoader) {
    return {
...
var ForeignObjectRenderer = function () {
    ...
    _createClass(ForeignObjectRenderer, [{
        ...
        value: function render(options) {
            ...
            options.logger.log('ForeignObject renderer initialized (' + options.width + 'x' + options.height + ' at ' + options.x + ',' + options.y + ') with scale ' + this.options.scale);
            var svg = createForeignObjectSVG(Math.max(options.windowWidth, options.width), Math.max(options.windowHeight, options.height), options.scrollX, options.scrollY, this.element);
            return loadSerializedSVG(svg).then(function (img) {
                if (options.backgroundColor) {
                    _this.ctx.fillStyle = options.backgroundColor.toString();
                    _this.ctx.fillRect(0, 0, options.width, options.height);
                }
                _this.ctx.drawImage(img, -options.x, -options.y);
                return _this.canvas;
            });
        }
    }]);
...
var NodeParser = exports.NodeParser = function NodeParser(node, imageLoader, logger) {
    if (true) {
    ...
    var container = new _NodeContainer2.default(node, null, imageLoader, index++);
    ...
    for (var childNode = node.firstChild, nextNode; childNode; childNode = nextNode) {
        ...
        if (childNode instanceof defaultView.Text || childNode instanceof Text || defaultView.parent && childNode instanceof defaultView.parent.Text) {
            ...
            if (IGNORED_NODE_NAMES.indexOf(childNode.nodeName) === -1) {
                var container = new _NodeContainer2.default(childNode, parent, imageLoader, index++);
            ...
            var _container = new _NodeContainer2.default(childNode, parent, imageLoader, index++);
...
var _ImageLoader = __webpack_require__(46);
